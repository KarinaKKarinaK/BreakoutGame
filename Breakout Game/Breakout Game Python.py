...
def init():
    ...
    for i in range(6):
        for j in range(10):
            bricks.append(Brick(10 + j * 79, 5 + i * 35, 70, 25, (190, 220, 255)))
...
def redrawGameWindow():
    ...
    if gameover:
        if len(bricks) == 0:
            resText = font.render("Congratulations!", 1, (255, 255, 255))
        else:
            resText = font.render("Nice, but try again!", 1, (255, 255, 255))
        win.blit(resText, ((sw//2 - resText.get_width()//2), sh//2 - resText.get_height()//2))
        playAgainText = font.render("Press Space to Play Again", 1, (190, 220, 255))
        win.blit(playAgainText, ((sw//2 - playAgainText.get_width()//2), sh//2 + 30))

    pygame.display.update()


player = Paddle(sw/2 - 50, sh - 100, 140, 20, (10, 255, 100))
...
while run:
    ...
    if gameover:
        if keys[pygame.K_SPACE]:
            ...
            for i in range(6):
                for j in range(10):
                    bricks.append(Brick(10 + j *79, 50 + i * 35, 70, 25, (190, 220, 255) ))
